...
!(function ($) {
	...
	var Iconpicker = function (element, options) {
		if (
			"undefined" == typeof $.fn.popover ||
			"undefined" == typeof $.fn.popover.Constructor.VERSION
		)
	...
	(Iconpicker.VERSION = "1.10.0"),
				...
				op.table
					...
					.on("click", function (e) {
						e.preventDefault(),
							...
							op.inline === !1
								? el.$element.popover(
										"3.x" === $.fn.bsVersion() ? "destroy" : "dispose"
								  )
								: op.table
										.find("i[class$='" + $(this).val() + "']")
										.parent()
										.addClass(op.selectedClass);
					}),
				op.table
					.find(".search-control")
					.off("keyup")
		...
		(Iconpicker.prototype.filterIcons = function () {
			var op = this.options,
				...
				icons = [];
			if (
				("lastest" != op.iconsetVersion &&
				"undefined" != typeof Iconpicker.ICONSET[op.iconset].allVersions
					? $.each(
							Iconpicker.ICONSET[op.iconset].allVersions,
							function (i, v) {
		...
		(Iconpicker.prototype.updateArrows = function (page) {
			...
			1 === page
				...
				page === total_pages || 0 === total_pages
				...
				for (var tr = $("<tr></tr>"), j = 0; j < op.cols; j++) {
					...
					if (pos < op.icons.length) {
						...
						btn
							.val(v)
							.attr("title", v)
							.append('<i class="' + op.iconClass + " " + v + '"></i>')
							.show(),
							op.icon === v &&
								btn
									.addClass(op.selectedClass)
									.addClass("btn-icon-selected");
					}
		...
		(Iconpicker.prototype.updateLabels = function (page) {
			...
			op.table
				...
				.html(
					op.labelHeader
						.replace("{0}", 0 === total_pages ? 0 : page)
						.replace("{1}", total_pages)
				);
			var offset = (page - 1) * this.totalIconsPerPage(),
				total = page * this.totalIconsPerPage();
			op.table.find(".icons-count").html(
				op.labelFooter
					.replace("{0}", total_icons ? offset + 1 : 0)
					.replace("{1}", total_icons > total ? total : total_icons)
					.replace("{2}", total_icons)
			),
				...
				for (var tr = $("<tr></tr>"), i = 0; i < op.cols; i++) {
					...
					if (0 === i || i === op.cols - 1) {
						var arrow = [
							...
							'<span class="' +
								(0 === i
									? op.arrowPrevIconClass
									: op.arrowNextIconClass) +
								'"></span>',
							"</button>",
						];
						td.append(arrow.join("")), tr.append(td);
					} else
						0 === tr.find(".page-count").length &&
							(td
								.attr("colspan", op.cols - 2)
								.append('<span class="page-count"></span>'),
							tr.append(td));
				}
		...
		(Iconpicker.prototype.updateSearch = function () {
			var op = this.options,
				search = [
					"<tr>",
					'   <td colspan="' + op.cols + '">',
					'       <input type="text" class="form-control search-control" style="width: ' +
						op.cols * ("3.x" === $.fn.bsVersion() ? 39 : 41) +
						'px;" placeholder="' +
						op.searchText +
						'">',
					"   </td>",
			...
			(search = $(search.join(""))),
				op.search === !0 ? search.show() : search.hide(),
		...
		(Iconpicker.prototype.setArrowClass = function (value) {
			this.options.arrowClass = this.removeAddClass(
				".btn-arrow",
		...
		(Iconpicker.prototype.setArrowNextIconClass = function (value) {
			this.options.arrowNextIconClass = this.removeAddClass(
				".btn-next > span",
		...
		(Iconpicker.prototype.setArrowPrevIconClass = function (value) {
			this.options.arrowPrevIconClass = this.removeAddClass(
				".btn-previous > span",
				this.options.arrowPrevIconClass,
				value
			);
		}),
		...
		(Iconpicker.prototype.setIconset = function (value) {
			var op = this.options;
			$.isPlainObject(value)
				? ((Iconpicker.ICONSET._custom = $.extend(
						Iconpicker.ICONSET_EMPTY,
						value
				  )),
				  (op.iconset = "_custom"))
				: Iconpicker.ICONSET.hasOwnProperty(value)
				? (op.iconset = value)
		...
		(Iconpicker.prototype.setSearch = function (value) {
			...
			value === !0 ? search.show() : search.hide(),
				search.val(""),
				this.changeList(1),
		...
		(Iconpicker.prototype.setSelectedClass = function (value) {
			this.options.selectedClass = this.removeAddClass(
				".btn-icon-selected",
		...
		(Iconpicker.prototype.setUnselectedClass = function (value) {
			this.options.unselectedClass = this.removeAddClass(
				".btn-icon",
				this.options.unselectedClass,
				value
			);
		});
		...
		return this.each(function () {
			...
			if (
				...
				if ("undefined" == typeof data[option])
					throw (
						'Iconpicker => The "' + option + '" method does not exists.'
					);
				data[option](params);
			} else {
				var op = data.options;
				op = $.extend(op, {
					...
					selected: -1,
					table: $(
						'<table class="table-icons"><thead></thead><tbody></tbody><tfoot></tfoot></table>'
					),
				});
				var name =
				...
				"BUTTON" === $this.prop("tagName")
					? ($this
							...
							.addClass(
								"iconpicker " +
									("3.x" === $.fn.bsVersion() ? "" : "dropdown-toggle")
							),
