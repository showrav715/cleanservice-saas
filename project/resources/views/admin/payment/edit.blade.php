                    ...
                    <form action="{{ route('admin.gateway.update', $paymentgateway) }}" method="POST"
                            ...
                            @if ($paymentgateway->information != null)
                                @foreach ($paymentgateway->convertAutoData() as $pkey => $pdata)
                                    @if ($pkey != 'sandbox_check')
                                        <div class="form-group row mb-3">
                                            <label
                                                class="col-sm-4 col-form-label">{{ __($paymentgateway->name . ' ' . ucwords(str_replace('_', ' ', $pkey))) }}
                                                *</label>
                                            <div class="col-sm-8">
                                    ...
                                    @if ($pkey == 'sandbox_check')
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" name="pkey[{{ __($pkey) }}]"
                                                class="custom-control-input" value="1" id="customCheck1"
                                                {{ $pdata == 1 ? 'checked' : '' }}>
                                            <label class="custom-control-label"
                                                for="customCheck1">{{ __($paymentgateway->name . ' ' . ucwords(str_replace('_', ' ', $pkey))) }}
                                                *</label>
                                        </div>
                                    @endif
                                @endforeach
                                ...
                                @foreach (DB::table('currencies')->get() as $dcurr)
                                    <input name="currency_id[]" {{ in_array($dcurr->code, $setCurrency) ? 'checked' : '' }}
                                        type="checkbox" id="currency_id{{ $dcurr->id }}" value="{{ $dcurr->code }}">
                        ...
                        @else
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    @foreach (DB::table('currencies')->get() as $dcurr)
                                        <input name="currency_id[]"
                                            {{ in_array($dcurr->code, $setCurrency) ? 'checked' : '' }} type="checkbox"
                                            id="currency_id{{ $dcurr->id }}" value="{{ $dcurr->code }}">
                                        <label class="mr-4"
                                            for="currency_id{{ $dcurr->id }}">{{ $dcurr->code }}</label>
                                    @endforeach
                                </div>
                            </div>
                        @endif
