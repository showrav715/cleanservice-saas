...
@section('content')
                            ...
                            @foreach ($brands as $item)
                                <tr>
                                    ...
                                    <td data-label="@lang('Photo')">
                                        <img src="{{ showPhoto($item->photo,$item->title) }}" alt="" width="100">
                                    </td>
                                    ...
                                    <td data-label="@lang('Action')" class="text-right">
                                        <a href="javascript:void()" class="btn btn-primary approve btn-sm edit mb-1"
                                            data-route="{{ route('seller.brand.update', $item->id) }}"
                                            data-item="{{ $item }}" data-path="{{sellerpath()}}" data-toggle="tooltip"
    ...
    <div class="modal fade" id="removeMod" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form action="{{ route('seller.brand.destroy') }}" method="POST">
                @method('DELETE')
                @csrf
