...
@section('breadcrumb')
 <section class="section">
    <div class="section-header justify-content-between">
        ...
        <a href="{{route('seller.language.index')}}" class="btn btn-primary"> <i class="fas fa-backward"></i> @lang('Back')</a>
    </div>
</section>
@endsection
...
<div class="row justify-content-center mt-3">
   <div class="col-lg-12">
      <div class="card mb-4">
         ...
         <div class="card-body">
            <form  action="{{route('seller.language.update',$language->id)}}" method="POST" enctype="multipart/form-data">
               @csrf
               @method('PUT')
               <div class="row">
                  ...
                  <div class="form-group col-md-4">
                    ...
                    <select class="form-control" name="direction" required>
                       <option value="ltr" {{$language->direction == 'ltr' ? 'selected' : ''}}>@lang('LTR')</option>
                       <option value="rtl" {{$language->direction == 'rtl' ? 'selected' : ''}}>@lang('RTL')</option>
                    </select>
                 </div> 
                  <div class="form-group col-md-4">
   ...
   <div class="col-md-12">
    <div class="card">
        <div class="card-header justify-content-between">
            ...
            <a href="javascript:void(0)" data-toggle="modal" data-target="#translate" class="btn btn-primary add"> <i class="fas fa-plus"></i> @lang('Add New')
                </a>
                    ...
                    <tbody class="custom-data">
                      ...
                      @forelse ($lang as $key => $value)
                        <tr class="elements">
                             ...
                             <td data-label="@lang('Value')" data-toggle="tooltip" title="{{$value}}">{{Str::limit($value,40)}}</td>
                             <td data-label="@lang('Action')" class="text-right">
                                 <a class="btn btn-primary btn-sm edit m-1" data-key="{{$key}}" data-value="{{$value}}" data-route="{{route('seller.translate.update',$language->id)}}" href="javascript:void(0)"><i class="fas fa-edit"></i></a>

                                 <a class="btn btn-danger btn-sm remove m-1" data-key="{{$key}}" data-value="{{$value}}" data-route="{{route('seller.translate.remove',$language->id)}}" href="javascript:void(0)"><i class="fas fa-trash"></i></a>
                             </td>
                        </tr>
                        @empty
...
<div class="modal fade" id="translate" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('seller.translate.store',$language->id)}}" method="post">
            @csrf
...
<div class="modal fade" id="remove" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('seller.translate.remove')}}" method="post">
            @csrf
